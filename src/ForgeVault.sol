...
contract ForgeVault is ERC4626, AccessControl, ReentrancyGuard, Pausable {
    ...
    constructor(IERC20 asset, address feeRecipient_, uint256 performanceFeeBps_)
        ERC4626(asset)
        ERC20("Forge Vault Token", "fvToken")
    {
    ...
    function executeStrategy(bytes calldata data) external onlyRole(STRATEGIST_ROLE) whenNotPaused {
    ...
    /// @return shares Amount of shares burned
    function withdrawWithSlippage(uint256 assets, address receiver, address owner, uint256 minShares)
        external
        returns (uint256 shares)
    {
